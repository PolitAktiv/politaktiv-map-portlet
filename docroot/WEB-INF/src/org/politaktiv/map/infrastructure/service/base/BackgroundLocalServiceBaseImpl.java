...
/**
 * The base implementation of the background local service.
 *
 * <p>
...
public abstract class BackgroundLocalServiceBaseImpl
	implements BackgroundLocalService, IdentifiableBean {
	/*
	 * NOTE FOR DEVELOPERS:
	 *
	...
	public Background addBackground(Background background)
		...
		background = backgroundPersistence.update(background, false);

		Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

		if (indexer != null) {
			try {
				indexer.reindex(background);
			}
	...
	public void deleteBackground(long backgroundId)
		...
		Background background = backgroundPersistence.remove(backgroundId);

		Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

		if (indexer != null) {
			try {
				indexer.delete(background);
			}
			catch (SearchException se) {
				if (_log.isWarnEnabled()) {
					_log.warn(se, se);
	...
	public void deleteBackground(Background background)
		throws SystemException {
		backgroundPersistence.remove(background);
	...
	public Background updateBackground(Background background)
		throws SystemException {
		return updateBackground(background, true);
	}
	 ...
	 * @throws SystemException if a system exception occurred
	 */
	public Background updateBackground(Background background, boolean merge)
		throws SystemException {
		background.setNew(false);

		background = backgroundPersistence.update(background, merge);
	...
	private static Log _log = LogFactoryUtil.getLog(BackgroundLocalServiceBaseImpl.class);
